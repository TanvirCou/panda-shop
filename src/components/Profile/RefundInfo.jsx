...
const RefundInfo = () => {
    ...
    useEffect(() => {
        const filtered = orders?.orders.filter(i => i?.status === "Processing for Refund" || i?.status === "Refund Success");
        setData(filtered);
    }, [orders]);
    ...
    return (
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 p-6 md:p-8">
            <h2 className="text-xl font-bold text-gray-900 mb-6">My Refunds</h2>
            ...
            {(!data || data.length === 0) && (
                <div className="flex flex-col items-center justify-center py-16 border-2 border-dashed border-gray-100 rounded-2xl text-center">
                    ...
                    <p className="font-semibold text-gray-400">No refunds yet</p>
                    <p className="text-sm text-gray-300 mt-1">Your refund requests will appear here</p>
                </div>
            )}
                                ...
                                return (
                                    <tr key={index} className="group hover:bg-gray-50/70 transition-colors">
                                        ...
                                        <td className="py-3.5 pr-4">
                                            <span className={`inline-flex items-center px-2.5 py-1 rounded-full text-xs font-semibold ${STATUS_STYLES[order.status] || "bg-gray-100 text-gray-600 border-gray-200"}`}>
                                                {order.status}
                                            </span>
                                        </td>
