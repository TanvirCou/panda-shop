...
const selectClass =
  "w-full h-11 px-3 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all";
const inputClass =
  "w-full h-11 px-4 bg-gray-50 border border-gray-200 rounded-xl text-sm text-gray-900 placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:border-transparent transition-all";
...
const AddressInfo = () => {
  ...
  const handleCreateAddress = async (e) => {
    ...
    try {
      await axios.put(
        "https://panda-shop-server-v3.up.railway.app/api/user/update-user-addresses",
  ...
  const handleDelete = async (id) => {
    try {
      await axios.delete(
        `https://panda-shop-server-v3.up.railway.app/api/user/delete-user-address/${id}`,
        { withCredentials: true }
      );
      ...
      {user?.user?.addresses?.length > 0 && (
        <div className='space-y-3'>
          {user.user.addresses.map((addr, index) => (
            <div
              key={index}
              className='flex flex-col sm:flex-row sm:items-center gap-3 p-4 rounded-xl bg-gray-50/70 border border-gray-100 hover:bg-gray-50 transition-colors'
            >
              <span
                className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-bold flex-shrink-0 ${
                  ADDRESS_TYPE_COLORS[addr.addressType] ||
                  "bg-gray-100 text-gray-600"
                }`}
              >
                {ADDRESS_TYPE_ICONS[addr.addressType] || "📌"}{" "}
                {addr.addressType}
              </span>
