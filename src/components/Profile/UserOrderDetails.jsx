...
const UserOrderDetails = () => {
  ...
  const reviewHandler = async (e) => {
    ...
    try {
      await axios.put(
        "https://panda-shop-server-production.up.railway.app/api/product/create-new-review",
        reviewDetails,
  ...
  const refundHandler = async (e) => {
    ...
    try {
      const res = await axios.put(
        `https://panda-shop-server-production.up.railway.app/api/order/order-refund/${id}`,
  ...
  const totalPrice = data?.cart.reduce((acc, i) => acc + i.discountPrice * i.qty, 0);
      ...
      <div className="max-w-4xl mx-auto px-4 sm:px-6 py-8">
        ...
        <div className="flex items-center gap-4 mb-8">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">
              #{data?._id?.slice(-10).toUpperCase()} · Placed on {data?.createdAt?.slice(0, 10)}
            </p>
          </div>
          {data?.status && (
            <span className={`ml-auto inline-flex items-center px-3 py-1.5 rounded-xl text-xs font-bold border ${STATUS_STYLES[data.status] || "bg-gray-100 text-gray-600 border-gray-200"}`}>
              {data.status}
            </span>
          )}
        </div>
        ...
        {statusIndex !== -1 && (
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wide mb-6">Order Progress</h3>
            <div className="relative flex justify-between items-start">
              ...
              <div 
                ...
                style={{ width: `${(statusIndex / (STATUS_STEPS.length - 1)) * 80}%` }}
              />

              {STATUS_STEPS.map((step, i) => (
                <div key={step} className="relative z-10 flex flex-col items-center w-1/3">
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center text-xs font-bold border-2 transition-all duration-300 ${
                    i <= statusIndex ? "bg-emerald-500 border-emerald-500 text-white shadow-lg shadow-emerald-200" : "bg-white border-gray-200 text-gray-400"
                  }`}>
                    {i < statusIndex ? "✓" : i + 1}
                  </div>
                  <span className={`text-xs font-bold mt-3 text-center transition-colors duration-300 ${
        ...
        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6 mb-6">
            ...
            {data?.cart.map((item, index) => (
              <div key={index} className="flex items-center gap-4 p-4 bg-gray-50 rounded-xl border border-gray-100">
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <p className="text-sm text-gray-500 mt-0.5">
                    ${item.discountPrice} × {item.qty} = <span className="font-bold text-gray-700">${item.discountPrice * item.qty}</span>
                  </p>
                </div>
          ...
          <div className="flex justify-end mt-4 pt-4 border-t border-gray-100">
            <div className="text-right">
              <p className="text-xs font-bold text-gray-400 uppercase tracking-wide">Order Total</p>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wide mb-4">Shipping Address</h3>
            <div className="space-y-2 text-sm text-gray-600">
              ...
              <p className="pt-1 font-medium text-gray-700">📞 {data?.user?.phoneNumber}</p>
            </div>
          </div>

          <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-6">
            <h3 className="text-sm font-bold text-gray-500 uppercase tracking-wide mb-4">Payment Info</h3>
            <div className="space-y-3">
              <div className="flex items-center justify-between">
                <span className="text-sm text-gray-500">Payment Status</span>
                <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${data?.paymentInfo?.status === "succeeded" ? "bg-emerald-50 text-emerald-700" : "bg-gray-100 text-gray-600"}`}>
