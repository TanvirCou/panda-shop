...
import { IoCartOutline, IoShieldCheckmarkOutline, IoStorefrontOutline, IoSwapHorizontalOutline } from "react-icons/io5";
              ...
              <div className="w-full lg:w-[48%] flex flex-col gap-5">
                ...
                <div className="relative bg-gradient-to-br from-gray-50 to-gray-100 rounded-3xl overflow-hidden aspect-square flex items-center justify-center border border-gray-100">
                  ...
                  {!eventData && (
                    ...
                    >
                      {click ? <IoMdHeart size={22} /> : <IoMdHeartEmpty size={22} />}
                    </button>
                  )}
                </div>
                ...
                <div className="grid grid-cols-3 gap-3 mt-1">
                  {[
                    { icon: <IoShieldCheckmarkOutline size={18} />, label: "Secure Payment" },
                    { icon: <IoSwapHorizontalOutline size={18} />, label: "Easy Returns" },
                    { icon: <IoStorefrontOutline size={18} />, label: "Verified Seller" },
                  ].map((b, i) => (
                    <div key={i} className="flex flex-col items-center gap-1.5 bg-gray-50 rounded-2xl py-3 px-2 border border-gray-100">
                      <span className="text-emerald-600">{b.icon}</span>
                      <span className="text-[11px] font-semibold text-gray-500 text-center leading-tight">{b.label}</span>
                    </div>
                  ))}
                </div>
              </div>
              ...
              <div className="w-full lg:w-[52%] flex flex-col gap-6">
                  ...
                  <div className="flex items-center gap-3 mt-2">
                    {data?.ratings > 0 && (
                      <div className="flex items-center gap-1">
                        {[...Array(5)].map((_, i) => (
                          <span key={i} className={`text-base ${i < Math.round(data.ratings) ? "text-amber-400" : "text-gray-200"}`}>★</span>
                        ))}
                        <span className="text-sm font-semibold text-gray-500 ml-1">({data?.reviews?.length || 0})</span>
                      </div>
                    )}
                    {data?.sold_out > 0 && (
                      <span className="text-xs font-semibold text-gray-400">{data.sold_out} sold</span>
                    )}
                  </div>
                </div>
                ...
                {data?.stock !== undefined && (
                  <div className="flex items-center gap-2">
                    <div className={`w-2 h-2 rounded-full ${data.stock > 0 ? "bg-emerald-500" : "bg-red-400"}`} />
