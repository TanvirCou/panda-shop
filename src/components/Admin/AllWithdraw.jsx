...
const AllWithdraw = () => {
    ...
    const fetchAllWithdraw = async () => {
        ...
        try {
            const res = await axios.get(
                "https://panda-shop-server-production.up.railway.app/api/withdraw/get-all-withdraw-request",
    ...
    const handleUpdate = async (e) => {
        ...
        try {
            await axios.put(
                `https://panda-shop-server-production.up.railway.app/api/withdraw//update-withdraw-request/${withdrawData._id}`,
    ...
    return (
        <div className="p-4 md:p-6 space-y-5">
            ...
            <div className="flex items-center gap-3">
                ...
                <div>
                    <h1 className="text-base font-black text-gray-900">Withdraw Requests</h1>
                    <p className="text-xs text-gray-400 font-medium">{withdraws.length} requests</p>
                </div>
            </div>
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                {withdraws.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-2">
                        ...
                        <p className="text-sm font-semibold text-gray-400">No withdrawal requests</p>
                    </div>
                ) : (
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {withdraws.map((item, index) => (
                                    <tr key={index} className="hover:bg-gray-50/60 transition-colors duration-150">
                                        ...
                                        <td className="px-5 py-3.5 font-semibold text-gray-800">{item?.shop?.name}</td>
