...
import { IoCalendarOutline, IoCallOutline, IoLocationOutline, IoLogOutOutline, IoSettingsOutline } from "react-icons/io5";
...
const ShopInfo = ({ data, products, id }) => {
  ...
  const handleLogout = async () => {
    await axios.get("https://panda-shop-server-production.up.railway.app/api/shop/logout", { withCredentials: true });
    dispatch(fetchShop());
    navigate("/");
  };

  const totalReview = products?.products.reduce((acc, i) => acc + i?.reviews.length, 0) || 0;
  const shopTotalRating = products?.products.reduce((acc, i) => acc + (i?.ratings || 0), 0) || 0;
  const shopAvgRating = totalReview > 0 ? (shopTotalRating / totalReview).toFixed(1) : null;
  const isOwner = shop?.shop?._id === id;
  const joinDate = data?.shop.createdAt ? new Date(data.shop.createdAt).toLocaleDateString("en-US", { month: "short", year: "numeric" }) : "—";
    ...
    <div className="bg-white rounded-2xl shadow-sm border border-gray-100">
      ...
      <div className="relative h-24 bg-gradient-to-r from-emerald-500 via-teal-500 to-emerald-400 rounded-t-2xl overflow-hidden">
        <div className="absolute inset-0 opacity-20"
          style={{ backgroundImage: "radial-gradient(circle at 30% 50%, white 1px, transparent 1px), radial-gradient(circle at 70% 50%, white 1px, transparent 1px)", backgroundSize: "20px 20px" }}
        />
      </div>
      ...
      <div className="flex justify-center -mt-10 mb-3 relative z-10">
        <div className="w-20 h-20 rounded-2xl overflow-hidden border-4 border-white shadow-lg bg-gray-50">
          {data?.shop.avatar && (
            <img src={data.shop.avatar} alt={data?.shop.name} className="w-full h-full object-cover" />
          )}
        </div>
      </div>
      ...
      <div className="text-center px-5 pb-4 border-b border-gray-100">
        ...
        {data?.shop.description && (
          <p className="text-xs text-gray-400 mt-1 leading-relaxed line-clamp-3">{data?.shop.description}</p>
        )}
      </div>
      ...
      <div className="grid grid-cols-3 divide-x divide-gray-100 border-b border-gray-100">
        {[
          ...
          <div key={stat.label} className="flex flex-col items-center py-3">
            ...
            <span className="text-[10px] text-gray-400 font-medium mt-0.5">{stat.label}</span>
          </div>
        ))}
      </div>
        ...
        {data?.shop.address && (
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-gray-700">{data.shop.address}</p>
            </div>
          </div>
        )}
        {data?.shop.phoneNumber && (
          <div className="flex items-start gap-3">
            ...
            <div>
              ...
              <p className="text-sm font-medium text-gray-700">{data.shop.phoneNumber}</p>
            </div>
          </div>
        )}
        <div className="flex items-start gap-3">
