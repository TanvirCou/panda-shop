        ...
        <div className="p-4 md:p-6 space-y-5">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    ...
                    <p className="text-sm text-gray-400 mt-0.5">{allEvents.length} event{allEvents.length !== 1 ? "s" : ""} in your shop</p>
                </div>
                ...
                {filtered.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-2">
                        ...
                        <p className="text-xs text-gray-300">
                            {allEvents.length === 0 ? "Create your first event to boost sales" : "Try a different search term"}
                        </p>
                        {allEvents.length === 0 && (
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {filtered.map((event, index) => {
                                    const statusCls = EVENT_STATUS_STYLES[event.status] || "bg-gray-50 text-gray-500 border-gray-100";
                                    return (
                                        <tr key={event._id} className="hover:bg-gray-50/50 transition-colors duration-150">
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex items-center gap-3">
                                                    ...
                                                    <p className="font-semibold text-gray-800 line-clamp-1 max-w-[180px]">{event.name}</p>
                                                </div>
                                            </td>
                                            <td className="px-5 py-4 font-bold text-gray-900">${event.discountPrice}</td>
                                            <td className="px-5 py-4">
                                            ...
                                            <td className="px-5 py-4 text-gray-500 font-medium">{event.sold_out}</td>
                                            <td className="px-5 py-4">
                                                <div className="text-xs text-gray-400 space-y-0.5">
                                                    {event.start_date && <p>Start: {new Date(event.start_date).toLocaleDateString()}</p>}
                                                    {event.end_date   && <p>End: {new Date(event.end_date).toLocaleDateString()}</p>}
                                                </div>
                                            </td>
                                            <td className="px-5 py-4">
                                            ...
                                            <td className="px-5 py-4">
                                                <div className="flex items-center justify-end gap-2">
                                                    <Link to={`/product/${event._id}?isEvent=true`}>
                                                        <button title="Preview" className="w-8 h-8 flex items-center justify-center rounded-lg bg-gray-50 hover:bg-cyan-50 text-gray-400 hover:text-cyan-600 border border-gray-100 hover:border-cyan-200 transition-all duration-200">
                                                            <FiEye size={14} />
                                                        </button>
                                                    </Link>
                                                    {confirmId === event._id ? (
                                                        <div className="flex items-center gap-1">
                                                            <button onClick={() => handleDelete(event._id)} className="px-2 py-1 text-[10px] font-bold bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors">Yes</button>
                                                        ...
                                                        <button title="Delete" onClick={() => setConfirmId(event._id)} className="w-8 h-8 flex items-center justify-center rounded-lg bg-gray-50 hover:bg-red-50 text-gray-400 hover:text-red-500 border border-gray-100 hover:border-red-200 transition-all duration-200">
