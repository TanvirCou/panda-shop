...
import { IoCallOutline, IoCardOutline, IoLocationOutline } from "react-icons/io5";
...
const OrderDetails = () => {
    ...
    const totalPrice = data?.cart.reduce((acc, i) => acc + i.discountPrice * i.qty, 0) || 0;
    const isRefundFlow = data?.status === "Processing for Refund" || data?.status === "Refund Success";
    const isFinalized = data?.status === "Delivered" || data?.status === "Refund Success";
        ...
        <div className="p-4 md:p-6 space-y-5">
            ...
            <div className="flex items-center justify-between gap-3 flex-wrap">
                <div>
                    ...
                    <p className="text-sm text-gray-400 mt-0.5 font-mono">#{data?._id?.slice(-12).toUpperCase()}</p>
                </div>
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                ...
                <div className="lg:col-span-2 space-y-4">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-4 flex items-center justify-between flex-wrap gap-3">
                        <div className="flex items-center gap-2">
                            ...
                            <div>
                                ...
                                <p className="text-sm font-bold text-gray-900">{data?.createdAt?.slice(0, 10)}</p>
                            </div>
                        </div>
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                        <div className="px-5 py-3.5 border-b border-gray-100">
                            <h2 className="text-sm font-bold text-gray-800">Order Items ({data?.cart.length})</h2>
                        </div>
                            ...
                            {data?.cart.map((item, index) => (
                                <div key={index} className="flex items-center gap-4 px-5 py-4">
                                    ...
                                    <div className="flex-1 min-w-0">
                                        ...
                                        <p className="text-xs text-gray-400 mt-0.5">${item.discountPrice} × {item.qty}</p>
                                    </div>
                    ...
                    {!isFinalized && (
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-5">
                            <h2 className="text-sm font-bold text-gray-800 mb-3">Update Order Status</h2>
                ...
                <div className="space-y-4">
                    ...
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-5">
                        <h2 className="text-sm font-bold text-gray-800 mb-4">Shipping Address</h2>
                        <div className="space-y-3">
                            <InfoRow
                                icon={IoLocationOutline}
                                label="Address"
                                value={`${data?.shippingAddress.address1 || ""} ${data?.shippingAddress.address2 || ""}`.trim()}
                            />
                            <InfoRow icon={IoLocationOutline} label="City" value={data?.shippingAddress.city} />
                            <InfoRow icon={IoLocationOutline} label="Country" value={data?.shippingAddress.country} />
                            <InfoRow icon={IoLocationOutline} label="Zip Code" value={data?.shippingAddress.zipCode} />
                            <InfoRow icon={IoCallOutline} label="Phone" value={data?.user?.phoneNumber || "—"} />
                        </div>
                    </div>

                    
                    <div className="bg-white rounded-2xl border border-gray-100 shadow-sm px-5 py-5">
                        <h2 className="text-sm font-bold text-gray-800 mb-4">Payment Info</h2>
                        <InfoRow
                            icon={IoCardOutline}
                        ...
                        {data?.paymentInfo?.type && (
                            <div className="mt-3">
                                <InfoRow icon={IoCardOutline} label="Method" value={data.paymentInfo.type} />
                            </div>
                        )}
                    </div>
                </div>
