...
const OrderDetails = () => {
  ...
  const handleStatus = async (e) => {
    ...
    try {
      await axios.put(
        `https://panda-shop-server-v4.up.railway.app/api/order/update-order-status/${data._id}`,
        { status },
  ...
  const refundHandler = async (e) => {
    ...
    try {
      const res = await axios.put(
        `https://panda-shop-server-v4.up.railway.app/api/order/order-refund-success/${data._id}`,
        { status },
        { withCredentials: true }
      );
