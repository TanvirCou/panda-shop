    ...
    const handleDelete = async (id) => {
        try {
            const res = await axios.delete(
                `https://panda-shop-server-production.up.railway.app/api/product/delete-product/${id}`,
        ...
        <div className="p-4 md:p-6 space-y-5">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    ...
                    <p className="text-sm text-gray-400 mt-0.5">{allProducts.length} products in your shop</p>
                </div>
                ...
                {filtered.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-2">
                        ...
                        <p className="text-sm font-semibold text-gray-400">
                            {allProducts.length === 0 ? "No products yet" : "No results found"}
                        </p>
                        <p className="text-xs text-gray-300">
                            {allProducts.length === 0 ? "Start by adding your first product" : "Try a different search term"}
                        </p>
                        {allProducts.length === 0 && (
                            <Link to="/shop/dashboard/create-product" className="mt-2">
                                <button className="flex items-center gap-1.5 text-xs font-semibold text-cyan-600 hover:text-cyan-500 transition-colors">
                                    Add Product <FiArrowRight size={12} />
                                </button>
                            </Link>
                        )}
                    </div>
                        ...
                        <table className="w-full text-sm">
                            <thead>
                                <tr className="bg-gray-50 text-xs font-bold text-gray-400 uppercase tracking-wide border-b border-gray-100">
                                    ...
                                    <th className="px-5 py-3.5 text-left">Original</th>
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {filtered.map((product, index) => (
                                    <tr key={product._id} className="hover:bg-gray-50/50 transition-colors duration-150">
                                        ...
                                        <td className="px-5 py-4 font-bold text-gray-900">${product.discountPrice}</td>
                                        <td className="px-5 py-4 text-gray-400 line-through text-xs">${product.originalPrice || "—"}</td>
                                        <td className="px-5 py-4">
                                        ...
                                        <td className="px-5 py-4 text-gray-500 font-medium">{product.sold_out}</td>
                                        <td className="px-5 py-4">
