...
const CouponCode = () => {
    ...
    const handleCreate = async (e) => {
        ...
        try {
            ...
            setOpen(false);
            setName(""); setValue(""); setMinAmount(""); setMaxAmount(""); setSelectedProduct("");
            fetchCouponCodes();
        } catch (error) {
        ...
        <div className="p-4 md:p-6 space-y-5">
            ...
            <div className="flex items-center justify-between flex-wrap gap-3">
                <div>
                    ...
                    <p className="text-sm text-gray-400 mt-0.5">{codes.length} coupon{codes.length !== 1 ? "s" : ""} active</p>
                </div>
                <button
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm overflow-hidden">
                {codes.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-20 gap-2">
                        ...
                        <p className="text-sm font-semibold text-gray-400">No coupon codes yet</p>
                        <p className="text-xs text-gray-300">Create your first discount code to attract more buyers</p>
                        <button onClick={() => setOpen(true)} className="mt-2 flex items-center gap-1.5 text-xs font-semibold text-cyan-600 hover:text-cyan-500 transition-colors">
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-gray-50">
                                {codes.map((coupon, index) => (
                                    <tr key={coupon._id} className="hover:bg-gray-50/50 transition-colors duration-150">
                                        ...
                                        <td className="px-5 py-4">
                                            <div className="flex items-center gap-1">
                                                ...
                                                <span className="font-bold text-emerald-600">{coupon.value}%</span>
                                            </div>
                                        </td>
                                        <td className="px-5 py-4 text-xs text-gray-400 font-medium">
                                            {coupon.minAmount ? `$${coupon.minAmount}` : "—"} / {coupon.maxAmount ? `$${coupon.maxAmount}` : "—"}
                                        </td>
                                        <td className="px-5 py-4 text-sm text-gray-600">
                                            {coupon.selectedProduct || <span className="text-gray-300 italic">All Products</span>}
                                        </td>
                                        <td className="px-5 py-4">
                                            <div className="flex items-center justify-end gap-2">
                                                {confirmId === coupon._id ? (
                                                    <div className="flex items-center gap-1">
                                                        <button onClick={() => handleDelete(coupon._id)} className="px-2 py-1 text-[10px] font-bold bg-red-500 text-white rounded-lg hover:bg-red-600 transition-colors">Yes</button>
                                                        <button onClick={() => setConfirmId(null)} className="px-2 py-1 text-[10px] font-bold bg-gray-200 text-gray-600 rounded-lg hover:bg-gray-300 transition-colors">No</button>
                                                    </div>
                                                ) : (
                                                    <button title="Delete" onClick={() => setConfirmId(coupon._id)} className="w-8 h-8 flex items-center justify-center rounded-lg bg-gray-50 hover:bg-red-50 text-gray-400 hover:text-red-500 border border-gray-100 hover:border-red-200 transition-all duration-200">
                ...
                <Modal onClose={() => setOpen(false)}>
                    <div className="p-6">
                        <div className="flex items-center gap-2 mb-1">
                            ...
                            <h2 className="text-lg font-black text-gray-900">Create Coupon Code</h2>
                        </div>
                        <p className="text-xs text-gray-400 mb-5">Set up a discount code for your customers</p>

                        <form onSubmit={handleCreate} className="space-y-4">
                            <FormField label="Coupon Code Name" required>
                                <input type="text" required value={name} onChange={e => setName(e.target.value.toUpperCase())} placeholder="e.g. SUMMER20" className={inputClass} />
                            </FormField>

                            <FormField label="Discount Percentage" required>
                                <div className="relative">
                                    <input type="number" required value={value} onChange={e => setValue(e.target.value)} placeholder="20" min="1" max="100" className={`${inputClass} pr-8`} />
                            ...
                            <div className="grid grid-cols-2 gap-3">
                                <FormField label="Min Order Amount">
                                    <div className="relative">
                                        ...
                                        <input type="number" value={minAmount} onChange={e => setMinAmount(e.target.value)} placeholder="0.00" className={`${inputClass} pl-7`} min="0" />
                                    </div>
                                </FormField>
                                <FormField label="Max Order Amount">
                                    <div className="relative">
                                        <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 font-bold text-sm">$</span>
                                        <input type="number" value={maxAmount} onChange={e => setMaxAmount(e.target.value)} placeholder="0.00" className={`${inputClass} pl-7`} min="0" />
                                    </div>
                                </FormField>
                            </div>
                            ...
                            {value > 0 && (
                                <div className="flex items-center gap-2 px-3 py-2 bg-cyan-50 rounded-xl border border-cyan-100">
                                    ...
                                    <span className="text-xs text-cyan-600 font-semibold">
                                        Customers save {value}% {selectedProduct ? `on "${selectedProduct}"` : "on all products"}
                                        {minAmount ? ` (min $${minAmount})` : ""}
                                    </span>
                                </div>
                            )}

                            <button type="submit" className="w-full h-11 bg-gradient-to-r from-cyan-600 to-sky-500 hover:from-cyan-500 hover:to-sky-400 text-white text-sm font-bold rounded-xl shadow-md transition-all duration-200 active:scale-[0.98]">
                                Create Coupon Code
                            </button>
                        </form>
                    </div>
