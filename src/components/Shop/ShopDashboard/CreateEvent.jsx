...
const CreateEvent = () => {
    ...
    const handleFile = (file) => {
        ...
        fetch("https://api.cloudinary.com/v1_1/ddcn60bx4/image/upload", { method: "POST", body: data })
            .then(res => res.json())
            .then(data => { setImgUrl(data.url.toString()); setUploading(false); })
            .catch(err => { console.error(err); setUploading(false); });
    };

    const removeImage = () => { setImages(null); setImgUrl(null); };

    const handleSubmit = async (e) => {
        e.preventDefault();
        if (!name || !discountPrice || !stock || !category || !startDate || !endDate) {
        ...
        try {
            ...
            setImages(null); setImgUrl(null); setStartDate(null); setEndDate(null);
        } catch (error) {
    ...
    return (
        <div className="p-4 md:p-6">
            ...
            <div className="mb-6">
                ...
                <p className="text-sm text-gray-400 mt-0.5">Set up a limited-time sale event for your shop</p>
            </div>
                    ...
                    <div className="lg:col-span-2 space-y-4">
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
                            ...
                            <FormField label="Event Name" required>
                                <input type="text" value={name} onChange={e => setName(e.target.value)} placeholder="Enter event name" className={inputClass} />
                            </FormField>

                            <FormField label="Description" required>
                                <textarea value={description} onChange={e => setDescription(e.target.value)} placeholder="Describe this event…" rows={4} className={`${inputClass} h-auto py-2.5 resize-none`} />
                            </FormField>
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                                ...
                                <FormField label="Tags">
                                    <input type="text" value={tags} onChange={e => setTags(e.target.value)} placeholder="e.g. flash-sale, summer" className={inputClass} />
                                </FormField>
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
                            <div className="flex items-center gap-2 pb-2 border-b border-gray-100">
                                ...
                                <p className="text-sm font-bold text-gray-800">Event Duration</p>
                                <span className="text-[10px] text-gray-400 ml-1">(min 3 days)</span>
                            </div>
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
                            <div className="flex items-center gap-2 pb-2 border-b border-gray-100">
                                ...
                                {uploading && <span className="text-[10px] text-amber-500 font-semibold ml-auto animate-pulse">Uploading…</span>}
                            </div>
                            <div className="flex flex-wrap gap-3">
                                {images ? (
                                    <div className="relative w-28 h-28 rounded-xl overflow-hidden border border-gray-100 bg-gray-50 group">
                                        <img src={URL.createObjectURL(images)} alt="" className="w-full h-full object-cover" />
                                        <button type="button" onClick={removeImage} className="absolute top-1 right-1 w-5 h-5 rounded-full bg-red-500 text-white flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                                            <FiX size={11} />
                                        </button>
                                    </div>
                                ) : (
                                    <label htmlFor="upload" className="w-28 h-28 rounded-xl border-2 border-dashed border-gray-200 bg-gray-50 hover:border-cyan-400 hover:bg-cyan-50 flex flex-col items-center justify-center cursor-pointer transition-all duration-200 gap-1.5">
                                        <LuImagePlus size={22} className="text-gray-400" />
                                        <span className="text-[10px] text-gray-400 font-medium">Upload Image</span>
                                        <input type="file" id="upload" onChange={e => handleFile(e.target.files[0])} className="hidden" accept="image/*" />
                                    </label>
                                )}
                            </div>
                        </div>
                        ...
                        <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5 space-y-4">
                            ...
                            <FormField label="Original Price">
                                <div className="relative">
                                    ...
                                    <input type="number" value={originalPrice} onChange={e => setOriginalPrice(e.target.value)} placeholder="0.00" className={`${inputClass} pl-7`} min="0" />
                                </div>
                            </FormField>
                            ...
                            <FormField label="Event Price (Discount)" required>
                                <div className="relative">
                                    <span className="absolute left-3.5 top-1/2 -translate-y-1/2 text-gray-400 text-sm font-bold">$</span>
                                    <input type="number" required value={discountPrice} onChange={e => setDiscountPrice(e.target.value)} placeholder="0.00" className={`${inputClass} pl-7`} min="0" />
                                </div>
                            </FormField>
                            ...
                            {originalPrice && discountPrice && Number(originalPrice) > Number(discountPrice) && (
                                <div className="flex items-center justify-between px-3 py-2 bg-emerald-50 rounded-xl border border-emerald-100">
                                    <span className="text-xs text-emerald-600 font-semibold">Savings</span>
                            ...
                            <FormField label="Stock Quantity" required>
                                <input type="number" required value={stock} onChange={e => setStock(e.target.value)} placeholder="0" className={inputClass} min="0" />
                            </FormField>
                        </div>
