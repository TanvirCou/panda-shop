...
const WithdrawMoney = () => {
    ...
    const [bankInfo, setBankInfo] = useState({
        bankName: "", bankCountry: "", bankSwiftCode: null,
        bankAccountNumber: null, bankHolderName: "", bankAddress: "",
    });
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            setOpen(false);
            setBankInfo({ bankName: "", bankCountry: "", bankSwiftCode: null, bankAccountNumber: null, bankHolderName: "", bankAddress: "" });
            dispatch(fetchShop());
        } catch (error) {
    ...
    const maskedAccount = shop?.shop?.withdrawMethod?.bankAccountNumber
        ? "*".repeat(String(shop.shop.withdrawMethod.bankAccountNumber).length - 3) + String(shop.shop.withdrawMethod.bankAccountNumber).slice(-3)
        : "";
    ...
    return (
        <div className="p-4 md:p-6 space-y-5">
            <div>
                ...
                <p className="text-sm text-gray-400 mt-0.5">Manage your payout method and request withdrawals</p>
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                ...
                <div className="bg-gradient-to-br from-cyan-600 via-sky-600 to-cyan-700 rounded-2xl p-6 relative overflow-hidden">
                    ...
                    <div className="relative z-10">
                        ...
                        <p className="text-purple-200/70 text-xs mt-1">After 10% service charge</p>
                        <button
                            onClick={() => balance < 50 ? toast.error("You need at least $50 to withdraw") : (setOpen(true), setShowBankForm(false))}
                    ...
                    {shop?.shop?.withdrawMethod ? (
                        <div className="space-y-3">
                            <div className="bg-gray-50 rounded-xl border border-gray-100 p-4 space-y-1.5">
                                ...
                                <p className="font-mono text-sm font-bold text-gray-800">{maskedAccount}</p>
                                <p className="text-sm text-gray-500">{shop.shop.withdrawMethod.bankName}</p>
                            </div>
                            <button
                        ...
                        <div className="flex flex-col items-center justify-center py-6 gap-2 text-center">
                            ...
                            <p className="text-sm font-semibold text-gray-400">No payout method</p>
                            <p className="text-xs text-gray-300">Add a bank account to receive withdrawals</p>
                            <button
                                onClick={() => { setOpen(true); setShowBankForm(true); }}
            ...
            <div className="bg-white rounded-2xl border border-gray-100 shadow-sm p-5">
                <p className="text-sm font-bold text-gray-800 mb-1">Transaction History</p>
                <p className="text-xs text-gray-400">Past withdrawal requests will appear here.</p>
            ...
            {open && (
                <Modal onClose={() => setOpen(false)}>
                    <div className="p-6">
                        {showBankForm ? (
                            <>
                                <h2 className="text-lg font-black text-gray-900 mb-1">Add Bank Account</h2>
                                <p className="text-xs text-gray-400 mb-5">Your details are securely stored</p>
                                <div className="space-y-4">
                                    <div className="grid grid-cols-2 gap-3">
                                        <FormField label="Bank Name" required>
                                            <input type="text" value={bankInfo.bankName} onChange={e => setBankInfo({ ...bankInfo, bankName: e.target.value })} placeholder="e.g. Citibank" className={inputClass} />
                                        </FormField>
                                        <FormField label="Bank Country" required>
                                            <input type="text" value={bankInfo.bankCountry} onChange={e => setBankInfo({ ...bankInfo, bankCountry: e.target.value })} placeholder="e.g. USA" className={inputClass} />
                                        </FormField>
                                    </div>
                                    <FormField label="Account Holder Name" required>
                                        <input type="text" value={bankInfo.bankHolderName} onChange={e => setBankInfo({ ...bankInfo, bankHolderName: e.target.value })} placeholder="Full legal name" className={inputClass} />
                                    </FormField>
                                    <FormField label="Account Number" required>
                                        <input type="text" value={bankInfo.bankAccountNumber || ""} onChange={e => setBankInfo({ ...bankInfo, bankAccountNumber: e.target.value })} placeholder="Account number" className={inputClass} />
                                    </FormField>
                                    <div className="grid grid-cols-2 gap-3">
                                        <FormField label="SWIFT Code" required>
                                            <input type="text" value={bankInfo.bankSwiftCode || ""} onChange={e => setBankInfo({ ...bankInfo, bankSwiftCode: e.target.value })} placeholder="SWIFT/BIC" className={inputClass} />
                                        </FormField>
                                        <FormField label="Bank Address" required>
                                            <input type="text" value={bankInfo.bankAddress} onChange={e => setBankInfo({ ...bankInfo, bankAddress: e.target.value })} placeholder="Branch address" className={inputClass} />
                                        </FormField>
                                    </div>
                                    <button onClick={handleSubmit} className="w-full h-11 bg-gradient-to-r from-cyan-600 to-sky-500 hover:from-cyan-500 hover:to-sky-400 text-white text-sm font-bold rounded-xl shadow-md transition-all duration-200 active:scale-[0.98]">
                                        Save Bank Account
                                    </button>
                                    <button onClick={() => setShowBankForm(false)} className="w-full h-9 text-sm font-semibold text-gray-400 hover:text-gray-600 transition-colors">
                                        ← Back
                                    </button>
                                </div>
                            </>
                            ...
                            <>
                                <h2 className="text-lg font-black text-gray-900 mb-1">Withdraw Funds</h2>
                                <p className="text-xs text-gray-400 mb-5">Minimum withdrawal: $50</p>
                                ...
                                {shop?.shop?.withdrawMethod ? (
                                    <div className="space-y-4">
                                        ...
                                        <div className="bg-gray-50 rounded-xl border border-gray-100 p-4 flex items-center justify-between">
                                            <div>
                                                <p className="text-xs text-gray-400 font-medium">Payout to</p>
                                                <p className="font-bold text-gray-800 text-sm">{shop.shop.withdrawMethod.bankName}</p>
                                                <p className="font-mono text-xs text-gray-500">{maskedAccount}</p>
                                            </div>
                                        ...
                                        <div className="bg-cyan-50 rounded-xl border border-cyan-100 px-4 py-2.5 flex items-center justify-between">
                                            <p className="text-xs text-cyan-600 font-semibold">Available</p>
                                            <p className="font-black text-cyan-700">${balance.toFixed(2)}</p>
                                        </div>
                                        ...
                                        <button onClick={handleWithdraw} className="w-full h-11 bg-gradient-to-r from-cyan-600 to-sky-500 hover:from-cyan-500 hover:to-sky-400 text-white text-sm font-bold rounded-xl shadow-md transition-all duration-200 active:scale-[0.98]">
                                            Request Withdrawal
                                        </button>
                                    </div>
                                ) : (
                                    <div className="text-center py-8 space-y-2">
                                        <span className="text-3xl">🏦</span>
                                        <p className="text-sm font-semibold text-gray-400">No payout method configured</p>
                                        <button onClick={() => setShowBankForm(true)} className="mt-2 px-4 py-2 text-xs font-bold text-cyan-600 border border-cyan-200 rounded-xl hover:bg-cyan-50 transition-colors">
                                            Add Bank Account
                                        </button>
                                    </div>
                                )}
