...
const Checkout = () => {
  ...
  const handleCode = async (e) => {
    ...
    const res = await axios.get(
      `https://panda-shop-server-v3.up.railway.app/api/coupon-code/get-coupon-value/${name}`
    );
  ...
  const totalPrice = couponCodeData
    ? (subTotal + shipping - discountPercentage).toFixed(2)
  ...
  const handleGoToPayment = (e) => {
    ...
    localStorage.setItem(
      "latestOrder",
      JSON.stringify({ shippingAddress, orderData })
    );
    setActive(2);
  };
  ...
  const paypalPaymentHandler = async (paymentInfo) => {
    ...
    try {
      const res = await axios.post(
        "https://panda-shop-server-v3.up.railway.app/api/order/create-order",
        order
      );
  ...
  const paymentHandler = async (e) => {
    ...
    try {
      const res = await axios.post(
        "https://panda-shop-server-v3.up.railway.app/api/payment/process",
        paymentData
      );
