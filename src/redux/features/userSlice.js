...
export const fetchUser = createAsyncThunk("user/fetchUser", async () => {
  const res = await axios.get(
    "https://panda-shop-server-v4.up.railway.app/api/user/get",
    {
...
export const fetchAllUser = createAsyncThunk("user/fetchAllUser", async () => {
  const res = await axios.get(
    "https://panda-shop-server-v4.up.railway.app/api/user/admin-all-users",
    { withCredentials: true }
  );
  return res.data;
});
...
export const updateUserInfo = createAsyncThunk(
  ...
  async (user) => {
    const res = await axios.put(
      "https://panda-shop-server-v4.up.railway.app/api/user/update-user-info",
      user,
      { withCredentials: true }
    );
    return res.data;
...
export const userSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchAllUser.fulfilled, (state, action) => {
      (state.allUsersLoading = false), (state.allUsers = action.payload);
    });
    builder.addCase(fetchAllUser.rejected, (state, action) => {
      (state.allUsersLoading = false), (state.error = action.payload);
    });
