...
export const fetchEvent = createAsyncThunk(
  ...
  async (shopId) => {
    const res = await axios.get(
      `https://panda-shop-server-production-v2.up.railway.app/api/event/all-events/${shopId}`
    );
    return res.data;
  }
);
...
export const deleteEvent = createAsyncThunk(
  ...
  async (eventId) => {
    const res = await axios.delete(
      `https://panda-shop-server-production-v2.up.railway.app/api/event/delete-event/${eventId}`,
...
export const fetchAllEvent = createAsyncThunk(
  ...
  async () => {
    const res = await axios.get(
      `https://panda-shop-server-production-v2.up.railway.app/api/event/get-all-events`
    );
    return res.data;
  }
);
