...
export const fetchShop = createAsyncThunk("shop/fetchShop", async () => {
  const res = await axios.get(
    "https://panda-shop-server-production-v2.up.railway.app/api/shop/get",
    {
...
export const fetchAllShop = createAsyncThunk("shop/fetchAllShop", async () => {
  const res = await axios.get(
    "https://panda-shop-server-production-v2.up.railway.app/api/shop/admin-all-shops",
    { withCredentials: true }
  );
  return res.data;
});
...
export const shopSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(fetchAllShop.pending, (state) => {
      (state.allShopLoading = true), (state.shop = []), (state.error = null);
    });
    ...
    builder.addCase(fetchAllShop.rejected, (state, action) => {
      (state.allShopLoading = false), (state.error = action.payload);
    });
    ...
    builder.addCase(logoutShop.rejected, (state, action) => {
      state.loading = false;
      state.error = action.payload;
    });
  },
